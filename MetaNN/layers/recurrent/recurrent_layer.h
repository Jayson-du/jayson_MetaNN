...
struct CheckPortOverLap_<TInputSet, LayerPortSet<TOutputPorts...>> {
  constexpr static bool value =
      (Set::HasKey<TInputSet, Previous<TOutputPorts>> && ...);
};
...
struct Wrapper2KernelInputMapHelper_ {
  ...
  using PermutePolicy = PolicyContainer<
      typename GetPermuteInfo_<ID, std::make_index_sequence<ValueDim>>::type>;
...
struct Wrapper2KernelInputMap_<LayerInMap<LayerKV<TKeys, TValues>...>,
                               TSeqIdCont> {
  using type =
      LayerInMap<LayerKV<TKeys, typename Wrapper2KernelInputMapHelper_<
                                    Map::Find<TSeqIdCont, TKeys>,
...
struct CalKernelInputMap_<LayerInMap<LayerKV<TKeys, TValues>...>, TSeqIDs> {
  static_assert((IsPreviousPort<TKeys> || ...),
                "No Previous port in the input port set.");
  using type =
      typename Wrapper2KernelInputMap_<LayerInMap<LayerKV<TKeys, TValues>...>,
                                       TSeqIDs>::type;
};
...
constexpr static bool InputMapPortsetMatch<
...
template <typename TInputMap, typename TPolicies> struct KernelGenerator_ {
  ...
  using Kernel =
      typename PolicySelect<LayerStructurePolicy,
                            WrapperPolicy>::template ActFunc<UInput, UPolicies>;
  static_assert(!LayerStructurePolicy::template IsDummyActFun<Kernel>,
                "Use PActFuncIs<...> to set kernel sublayer.");

  using KernelPolicy = SubPolicyPicker<TPolicies, KernelSublayer>;
  constexpr static bool IsUpdate =
      PolicySelect<GradPolicy, KernelPolicy>::IsUpdate;
  constexpr static bool UseBptt =
      PolicySelect<RnnPolicy, KernelPolicy>::UseBptt;
  constexpr static bool UpdateFeedbackOutput =
      PolicySelect<GradPolicy, WrapperPolicy>::IsFeedbackOutput ||
      (IsUpdate && UseBptt);

  using AmendKernelPolicy =
      typename std::conditional_t<UpdateFeedbackOutput,
  ...
  using KernelInputMap =
      typename CalKernelInputMap_<TInputMap, SeqIdCont>::type;
  ...
  using InputMap =
      typename std::conditional_t<std::is_same_v<KernelInputMap, NullParameter>,
                                  EmptyLayerInMap_<InputPortSet>,
                                  Identity_<TInputMap>>::type;
  static_assert(InputMapPortsetMatch<InputMap, InputPortSet>,
                "Invalid input port set.");
...
struct ShapeDictHelper<true, LayerInMap<LayerKV<TKeys, TValues>...>> {
  using shapeDictType = typename VarTypeDict<TKeys...>::template Values<
      RemConstRef<decltype(std::declval<TValues>().Shape())>...>;
  ...
  static void PickShapeInfoHelper(const TIn &p_in, Cont &&p_cont) {
    ...
    static_assert(
        std::is_same_v<decltype(curShape),
                       typename shapeDictType::template ValueType<Key>>);
    ...
    if constexpr (Sequential::Size<TKeysCont> == 0)
      ...
      if constexpr (Map::HasKey<TSeqIdCont, CurType>) {
        ...
        constexpr static size_t ValueDim =
            DataCategory<decltype(oriValue)>::DimNum;
        using PermutePolicy =
            typename GetPermuteInfo_<seqID,
                                     std::make_index_sequence<ValueDim>>::type;
        auto newValue = PermuteInv<PolicyContainer<PermutePolicy>>(oriValue);
        assert(newValue.Shape() == p_shape.template Get<CurType>());
        auto newCont =
            std::forward<TCont>(p_cont).template Set<CurType>(newValue);
        return CollapseHelper<Sequential::Tail<TKeysCont>, TSeqIdCont>(
            p_shape, std::move(newCont));
      } else if constexpr (!IsPreviousPort<CurType>) {
        auto newValue =
            ReduceSum<PolicyContainer<PModifyDimNumIs<1>>>(oriValue);
        assert(newValue.Shape() == p_shape.template Get<CurType>());
        auto newCont =
            std::forward<TCont>(p_cont).template Set<CurType>(newValue);
        return CollapseHelper<Sequential::Tail<TKeysCont>, TSeqIdCont>(
        ...
        return CollapseHelper<Sequential::Tail<TKeysCont>, TSeqIdCont>(
            p_shape, std::forward<TCont>(p_cont));
      }
    }
  }
  ...
  static auto Collapse(type &shapeStack, TRes &&p_res) {
    ...
    shapeStack.pop();
    return CollapseHelper<VarTypeDict<TKeys...>, TSeqIdCont>(
        currentShapeDict, std::forward<TRes>(p_res));
  }
};
...
auto PermuteBySeqID(const TInput &p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    if constexpr (Map::HasKey<TSeqID, CurType>) {
      ...
      constexpr static size_t ValueDim =
          DataCategory<decltype(curValue)>::DimNum;
      using PermutePolicy =
          typename GetPermuteInfo_<curSeqID,
                                   std::make_index_sequence<ValueDim>>::type;
      auto newValue =
          Permute<PolicyContainer<PermutePolicy>>(std::move(curValue));
      auto newOutput =
          std::forward<TOutput>(p_output).template Set<CurType>(newValue);
      return PermuteBySeqID<Sequential::Tail<TKeyCont>, TSeqID>(
          p_input, std::move(newOutput));
    } else {
      auto newOutput =
          std::forward<TOutput>(p_output).template Set<CurType>(curValue);
      return PermuteBySeqID<Sequential::Tail<TKeyCont>, TSeqID>(
          p_input, std::move(newOutput));
    }
  }
...
auto Split0(const TInput &p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    if constexpr (Map::HasKey<TSeqID, CurType>) {
      ...
      auto newOutput =
          std::forward<TOutput>(p_output).template Set<CurType>(inputValue);
      return Split0<Sequential::Tail<TKeyCont>, TSeqID>(p_input,
                                                        std::move(newOutput));
    } else if constexpr (IsPreviousPort<CurType>) {
...
auto SplitN(const TInput &p_input, TOutput &&p_output,
  ...
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    if constexpr (IsPreviousPort<CurType>) {
      ...
      auto newOutput =
          std::forward<TOutput>(p_output).template Set<CurType>(inputValue);
      return SplitN<Sequential::Tail<TKeyCont>, TSeqID>(
          p_input, std::move(newOutput), p_previous, id);
    } else {
      auto curValue = p_input.template Get<CurType>();
      if constexpr (Map::HasKey<TSeqID, CurType>) {
...
auto InitOutputCont(TInput &&p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    using CurType = Sequential::Head<TKeyCont>;
    using CurValueType =
        typename RemConstRef<TInput>::template ValueType<CurType>;
    ScalableTensor<CurValueType> aimValue;
    aimValue.PushBack(std::forward<TInput>(p_input).template Get<CurType>());
    auto newOutput =
        std::forward<TOutput>(p_output).template Set<CurType>(aimValue);
    return InitOutputCont<Sequential::Tail<TKeyCont>>(
        std::forward<TInput>(p_input), std::move(newOutput));
  }
}
...
void FillOutputCont(TInput &&p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    using CurType = Sequential::Head<TKeyCont>;
    p_output.template Get<CurType>().PushBack(
        std::forward<TInput>(p_input).template Get<CurType>());
    return FillOutputCont<Sequential::Tail<TKeyCont>>(
        std::forward<TInput>(p_input), std::forward<TOutput>(p_output));
  }
}
...
auto GradSplit0(const TInput &p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    auto newOutput = std::forward<TOutput>(p_output).template Set<CurType>(
        MakeDynamic(std::move(inputValue)));
    return GradSplit0<Sequential::Tail<TKeyCont>>(p_input,
                                                  std::move(newOutput));
  }
}
...
auto GradSplitN(const TInput &p_input, const TPrevious &p_previous,
  ...
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    if constexpr (!UseBptt) {
      auto newOutput = std::forward<TOutput>(p_output).template Set<CurType>(
          MakeDynamic(std::move(inputValue)));
      return GradSplitN<Sequential::Tail<TKeyCont>, UseBptt>(
          p_input, p_previous, std::move(newOutput), id);
    } else {
      auto amendInput =
          p_previous.template Get<Previous<CurType>>() + inputValue;
      auto newOutput = std::forward<TOutput>(p_output).template Set<CurType>(
          MakeDynamic(std::move(amendInput)));
      return GradSplitN<Sequential::Tail<TKeyCont>, UseBptt>(
          p_input, p_previous, std::move(newOutput), id);
    }
  }
...
auto ReverseOutputCont(TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    return ReverseOutputCont<Sequential::Tail<TKeyCont>>(
...
void FillNormalGradOutput(TInput &&p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    return FillNormalGradOutput<Sequential::Tail<TKeyCont>>(
        std::forward<TInput>(p_input), std::forward<TOutput>(p_output));
  }
}
...
auto FillPrevGradOutput(TInput &&p_input, TOutput &&p_output) {
  if constexpr (Sequential::Size<TKeyCont> == 0)
    ...
    if constexpr (IsPreviousPort<CurType>) {
      auto newOutput = std::forward<TOutput>(p_output).template Set<CurType>(
          std::forward<TInput>(p_input).template Get<CurType>());
      return FillPrevGradOutput<Sequential::Tail<TKeyCont>>(
          std::forward<TInput>(p_input), std::move(newOutput));
    } else {
      return FillPrevGradOutput<Sequential::Tail<TKeyCont>>(
          std::forward<TInput>(p_input), std::forward<TOutput>(p_output));
...
public:
    ...
    if constexpr (IsFeedbackOutput) {
      if (!m_inputShapeStack.empty()) {
        throw std::runtime_error(
            "[RecurrentLayer] NeutralInvariant fails: ShapeStack is not "
            "empty.");
      }
    }
  ...
  template <typename TIn> auto FeedForward(TIn &&p_in) {
    ...
    auto permuteRes = NSRecurrentLayer::PermuteBySeqID<TInputKeys, SeqIdCont>(
        std::forward<TIn>(p_in), TInputKeys::Create());
    const size_t seqNum =
        NSRecurrentLayer::GetSeqNum<TInputKeys, SeqIdCont>(permuteRes);
    if (seqNum == 0) {
      throw std::runtime_error("Empty sequence as input.");
    }

    auto firstInputCont = NSRecurrentLayer::Split0<TInputKeys, SeqIdCont>(
        permuteRes, TInputKeys::Create());
    auto previousOutput = m_kernel.FeedForward(std::move(firstInputCont));
    using OutputKeys = typename decltype(previousOutput)::Keys;
    auto outputCont = NSRecurrentLayer::InitOutputCont<OutputKeys>(
        previousOutput, OutputKeys::Create());

    for (size_t i = 1; i < seqNum; ++i) {
      auto curInputCont = NSRecurrentLayer::SplitN<TInputKeys, SeqIdCont>(
          permuteRes, TInputKeys::Create(), previousOutput, i);
  ...
  template <typename TIn> auto FeedBackward(TIn &&p_in) {
    ...
    if constexpr (!IsFeedbackOutput && !IsUpdate) {
      ...
      auto curInputCont =
          NSRecurrentLayer::GradSplit0<TInputKeys>(p_in, TInputKeys::Create());
      auto curOutputCont = m_kernel.FeedBackward(std::move(curInputCont));

      for (size_t i = 2; i <= seqNum; ++i) {
        auto curInputCont = NSRecurrentLayer::GradSplitN<TInputKeys, UseBptt>(
            p_in, curOutputCont, TInputKeys::Create(), seqNum - i);
      ...
      auto outputGrad = NSRecurrentLayer::InitOutputCont<OutputGradKeys>(
          curOutputCont, OutputGradKeys::Create());
      for (size_t i = 2; i <= seqNum; ++i) {
        auto curInputCont = NSRecurrentLayer::GradSplitN<TInputKeys, UseBptt>(
            p_in, curOutputCont, TInputKeys::Create(), seqNum - i);
        curOutputCont = m_kernel.FeedBackward(std::move(curInputCont));
        NSRecurrentLayer::FillNormalGradOutput<OutputGradKeys>(curOutputCont,
                                                               outputGrad);
      }

      auto filledPrevGrad =
          NSRecurrentLayer::FillPrevGradOutput<OutputGradKeys>(
              curOutputCont, std::move(outputGrad));

      NSRecurrentLayer::ReverseOutputCont<OutputGradKeys>(filledPrevGrad);
      return TShapeDickHelper::template Collapse<SeqIdCont>(
          m_inputShapeStack, std::move(filledPrevGrad));
    }
  }
