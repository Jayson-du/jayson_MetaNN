 ...
 public:
  ...
  auto FeedForward(TIn&& p_in) {
    ...
    static_assert(DataCategory<decltype(res)>::DimNum ==
                  DataCategory<decltype(input1)>::DimNum +
                      DataCategory<decltype(input2)>::DimNum - modDimNum * 2);

    return LayerOutputCont<DotLayer>().template Set<LayerOutput>(
        std::move(res));
  }
  ...
  auto FeedBackward(TGrad&& p_grad) {
    if constexpr (!IsFeedbackOutput ||
      ...
      static_assert(DataCategory<decltype(grad)>::DimNum ==
                        DataCategory<TLeftOperandFP>::DimNum +
                            DataCategory<TRightOperandFP>::DimNum -
                            modDimNum * 2,
                    "Grad has mismatch dimension.");

      using PPermute1 = typename NSDotLayer::ModeDim2PermuteArr_<
          DataCategory<TRightOperandFP>::DimNum, modDimNum>::type;
      constexpr size_t grad1DotDim =
          DataCategory<TRightOperandFP>::DimNum - modDimNum;
      auto grad1 = Dot<PolicyContainer<PModifyDimNumIs<grad1DotDim>>>(
          grad, Permute<PolicyContainer<PPermute1>>(input2));

      constexpr size_t grad2DotDim =
          DataCategory<TLeftOperandFP>::DimNum - modDimNum;
      using PPermute2 = typename NSDotLayer::ModeDim2PermuteArr_<
          DataCategory<TLeftOperandFP>::DimNum, grad2DotDim>::type;
      auto grad2 = Dot<PolicyContainer<PModifyDimNumIs<grad2DotDim>>>(
          Permute<PolicyContainer<PPermute2>>(input1), grad);
      ...
      LayerTraits::PopoutFromStack(m_input1, m_input2, m_inputShape1,
                                   m_inputShape2);
      return LayerInputCont<DotLayer>()
          .template Set<LeftOperand>(std::move(grad1))
