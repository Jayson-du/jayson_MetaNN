...
class NLLLossLayer {
 ...
 public:
  ...
  auto FeedForward(TIn&& p_in) {
    ...
    return LayerOutputCont<NLLLossLayer>().template Set<LayerOutput>(
        std::move(res));
  }
  ...
  auto FeedBackward(TGrad&& p_grad) {
    if constexpr (IsFeedbackOutput) {
      ...
      auto input = m_input.top();
      auto weight = m_weight.top();
      auto grad = std::forward<TGrad>(p_grad).template Get<LayerOutput>();
      auto res =
          NLLLossGrad(std::move(grad), std::move(weight), std::move(input));
      m_inputShape.CheckDataShape(res);

      LayerTraits::PopoutFromStack(m_input, m_weight, m_inputShape);
      return LayerInputCont<NLLLossLayer>().template Set<LayerInput>(
          std::move(res));
    } else {
