...
class SoftmaxLayer {
 ...
 public:
  ...
  auto FeedForward(TIn&& p_in) {
    ...
    return LayerOutputCont<SoftmaxLayer>().template Set<LayerOutput>(
        std::move(res));
  }
  ...
  auto FeedBackward(TGrad&& p_grad) {
    if constexpr (!IsFeedbackOutput ||
      ...
      auto res =
          SoftmaxGrad<CurLayerPolicy>(std::move(grad), std::move(softmaxRes));
      m_inputShape.CheckDataShape(res);

      LayerTraits::PopoutFromStack(m_data, m_inputShape);
      return LayerInputCont<SoftmaxLayer>().template Set<LayerInput>(
          std::move(res));
    }
  }
