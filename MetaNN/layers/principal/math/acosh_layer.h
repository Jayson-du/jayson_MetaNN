...
namespace MetaNN {
template <typename TInputs, typename TPolicies> class AcoshLayer {
...
public:
  AcoshLayer(std::string name) : m_name(std::move(name)) {}
  ...
  template <typename TIn> auto FeedForward(TIn &&p_in) {
    ...
    return LayerOutputCont<AcoshLayer>().template Set<LayerOutput>(
        std::move(res));
  }
  ...
  template <typename TGrad> auto FeedBackward(TGrad &&p_grad) {
    if constexpr (!IsFeedbackOutput ||
      ...
      LayerTraits::PopoutFromStack(m_data, m_inputShape);
      return LayerInputCont<AcoshLayer>().template Set<LayerInput>(
          std::move(res));
    }
  }
