...
namespace MetaNN {
template <typename TInputs, typename TPolicies> class CosLayer {
...
public:
  CosLayer(std::string name) : m_name(std::move(name)) {}
  ...
  template <typename TIn> auto FeedForward(TIn &&p_in) {
    ...
    return LayerOutputCont<CosLayer>().template Set<LayerOutput>(
        std::move(res));
  }
  ...
  template <typename TGrad> auto FeedBackward(TGrad &&p_grad) {
    if constexpr (!IsFeedbackOutput ||
      ...
      LayerTraits::PopoutFromStack(m_data, m_inputShape);
      return LayerInputCont<CosLayer>().template Set<LayerInput>(
          std::move(res));
    }
  }
