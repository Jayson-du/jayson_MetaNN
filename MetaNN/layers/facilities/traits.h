...
void ParamGradCollect(const std::string &name, const TWeight &weight,
  ...
  if (stackSize == 1) {
    ...
    ScalableTensor<RemConstRef<typename TGradStack::value_type>> dBatch(
        weight.Shape());
...
template <typename TVarTypeDictOutter, typename TVarTypeDictInner,
          typename... TKVs>
...
template <typename TVarTypeDictOutter, typename TVarTypeDictInner,
          typename TCur, typename... TKVs>
struct CreateVarTypeDict_<TVarTypeDictOutter, TVarTypeDictInner, TCur,
                          TKVs...> {
  using NewOutter =
      Sequential::PushBack<TVarTypeDictOutter, typename TCur::KeyType>;
  using NewInner =
      Sequential::PushBack<TVarTypeDictInner, typename TCur::ValueType>;
...
struct VarTypeDict2IOMap_<TVarTypeDict, VarTypeDict<TKeys...>> {
  using type = LayerInMap<
      LayerKV<TKeys, typename TVarTypeDict::template ValueType<TKeys>>...>;
};
...
struct LayerInMapForwardTransfer_<TLayer, LayerInMap<TKVs...>> {
  using TVarTypeDictFill =
      typename CreateVarTypeDict_<std::tuple<>, std::tuple<>, TKVs...>::type;
  using TForwardRes = decltype(std::declval<TLayer>().FeedForward(
      std::declval<TVarTypeDictFill>()));
...
using LayerOutputItemTypes =
    typename NSLayerInMapTrasfer::LayerInMapForwardTransfer_<
        TLayer, typename TLayer::InputMap>::type;
...
template <typename TData, typename TInput> auto Collapse(TInput &&p_input) {
  ...
  if constexpr (IsValidCategoryTag<AimCategory>) {
    ...
    if constexpr (OriDim == AimDim) {
      ...
      return ReduceSum<PolicyContainer<PModifyDimNumIs<OriDim - AimDim>>>(
          std::forward<TInput>(p_input));
    }
  } else {
