...
using Topology = ComposeTopology<
    ...
    Sublayer<AddSublayer, AddLayer>, Sublayer<DotSublayer, DotLayer>,
...
class LinearLayer : public NSLinearLayer::Base<TInputs, TPolicies> {
  ...
  using WeightParamType =
      typename TBase::template SublayerType<WeightParamSublayer>::ParamType;
  using BiasParamType =
      typename TBase::template SublayerType<BiasParamSublayer>::ParamType;
  using WeightParamShapeType =
      RemConstRef<decltype(std::declval<WeightParamType>().Shape())>;
  ...
  LinearLayer(const std::string& p_name, WeightParamShapeType weightShape,
      ...
      : TBase(TBase::CreateSublayers()
                  .template Set<WeightParamSublayer>(p_name + "/weight",
                                                     std::move(weightShape))
                  .template Set<BiasParamSublayer>(p_name + "/bias",
                                                   std::move(biasShape))
