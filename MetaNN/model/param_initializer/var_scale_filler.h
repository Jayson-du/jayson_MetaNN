...
auto GetFanInFanOut(const Tensor<TElem, TDevice, uDim> &t) {
  if constexpr (uDim == 1) {
    ...
    static_assert(DependencyFalse<Tensor<TElem, TDevice, uDim>>,
                  "Dimension is 0.");
  }
}
} // namespace NSVarScaleFiller
...
public:
  ...
  template <typename TData> void Fill(TData &data) {
    ...
    if constexpr (std::is_same<
                      ScaleMode,
                      VarScaleFillerPolicy::ScaleModeTypeCate::FanIn>::value) {
      fan_factor = static_cast<double>(fanIn);
    } else if constexpr (std::is_same<ScaleMode,
                                      VarScaleFillerPolicy::ScaleModeTypeCate::
                                          FanOut>::value) {
      fan_factor = fanOut;
    } else if constexpr (std::is_same<ScaleMode,
                                      VarScaleFillerPolicy::ScaleModeTypeCate::
    ...
    using DistType =
        typename PolicySelect<VarScaleFillerPolicy, TPolicyCont>::Distribute;
    using ElementType = typename TData::ElementType;
    if constexpr (std::is_same<DistType,
                               VarScaleFillerPolicy::DistributeTypeCate::
                                   Uniform>::value) {
      ElementType limit =
          static_cast<ElementType>(sqrt(3.0 * m_factor / fan_factor));
      std::uniform_real_distribution<ElementType> dist(-limit, limit);
      NSInitializer::FillWithDist(data, dist, m_engine);
    } else if constexpr (std::is_same<DistType,
                                      VarScaleFillerPolicy::DistributeTypeCate::
                                          Norm>::value) {
      double stddev = sqrt(m_factor / fan_factor);
      std::normal_distribution<ElementType> dist(
          0, static_cast<ElementType>(stddev));
      NSInitializer::FillWithDist(data, dist, m_engine);
    } else {
...
class MSRAFiller
    : public VarScaleFiller<NSVarScaleFiller::MSRAFillerPolicy<TPolicyCont>> {
  using BaseType =
