...
template <typename T> struct Cont : BaseCont {
...
class WeightBuffer {
public:
  template <typename TType> NSWeightBuffer::Cont<TType> *GetCont() const {
  ...
  template <typename T> const T *TryGet(const std::string &name) const {
  ...
  template <typename T> T *TryGet(const std::string &name) {
    auto *cont = GetCont<T>();
    if (!cont)
      return nullptr;
  ...
  template <typename TType> bool IsExist(const std::string &name) const {
...
private:
  std::unordered_map<size_t, std::unique_ptr<NSWeightBuffer::BaseCont>>
      m_params;
};
} // namespace MetaNN
