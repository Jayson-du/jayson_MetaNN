...
struct MinorCheck_<PolicyContainer<TCurPolicy, TP...>> {
  static constexpr bool cur_check =
      ((!std::is_same_v<typename TCurPolicy::MinorClass,
...
using PolicySelect =
    typename NSPolicySelect::Selector_<TMajorClass, TPolicyContainer>::type;
...
using SubPolicyPicker =
    typename NSSubPolicyPicker::SubPolicyPicker_<TPolicyContainer,
                                                 TLayerName>::type;
...
template <typename TNewPolicy> struct Filter_ {
  ...
  struct apply : std::conditional_t<
                     std::is_same_v<typename TInput::MajorClass, TNewMajor> &&
                         std::is_same_v<typename TInput::MinorClass, TNewMinor>,
                     Identity_<TState>, Sequential::PushBack_<TState, TInput>> {
  };
...
struct PickPolicyOjbect_<PolicyContainer<TPolicies...>, TMajorClass,
                         TMinorClass> {
  using type = TMajorClass;
};

template <typename TMajorClass, typename TMinorClass, typename TCurPolicy,
          typename... TPolicies>
struct PickPolicyOjbect_<PolicyContainer<TCurPolicy, TPolicies...>, TMajorClass,
                         TMinorClass> {
  constexpr static bool IsThePolicy =
      std::is_same_v<typename TCurPolicy::MajorClass, TMajorClass> &&
      std::is_same_v<typename TCurPolicy::MinorClass, TMinorClass>;
  using type = typename std::conditional_t<
      IsThePolicy, Identity_<TCurPolicy>,
      PickPolicyOjbect_<PolicyContainer<TPolicies...>, TMajorClass,
                        TMinorClass>>::type;
};

template <typename TPolicyContainer, typename TMajorClass, typename TMinorClass>
using PickPolicyOjbect =
    typename PickPolicyOjbect_<TPolicyContainer, TMajorClass,
                               TMinorClass>::type;
...
template <typename TMajorClass, typename TMinorClass, typename TCurPolicy,
          typename... TPolicies>
struct HasNonTrivialPolicy_<PolicyContainer<TCurPolicy, TPolicies...>,
                            TMajorClass, TMinorClass> {
  constexpr static bool IsThePolicy =
      std::is_same_v<typename TCurPolicy::MajorClass, TMajorClass> &&
      std::is_same_v<typename TCurPolicy::MinorClass, TMinorClass>;
  constexpr static bool value =
      OrValue<IsThePolicy, HasNonTrivialPolicy_<PolicyContainer<TPolicies...>,
                                                TMajorClass, TMinorClass>>;
};
