 ...
 public:
  ...
  ScalableTensor(TIterator b, TIterator e)
    ...
    if (b == e) {
      throw std::runtime_error(
          "Cannot initialize dynamic array with empty sequence.");
    }
  ...
  void PushBack(TData data) {
    ...
    if constexpr (ElemShapeDim != 0) {
      if (!std::equal(std::begin(m_shape) + 1, std::end(m_shape),
                      std::begin(data.Shape()))) {
  ...
  auto EvalRegister() const {
    if (!m_evalBuf.IsEvaluated()) {
      ...
      if (!EvalPlan::Inst().IsAlreadyRegisted(outHandle.DataPtr())) {
        ...
        using ItemType =
            NSScalableTensor::EvalItem<TOpEvalHandle, decltype(outHandle),
                                       ElemShapeDim + 1>;
        using GroupType =
            NSScalableTensor::EvalGroup<TOpEvalHandle, decltype(outHandle),
                                        ElemShapeDim + 1>;
        using DispatcherType = TrivialEvalItemDispatcher<GroupType>;

        auto item = std::make_unique<ItemType>(std::move(handleBuf),
                                               std::move(outHandle),
                                               std::move(depVec), m_shape);
