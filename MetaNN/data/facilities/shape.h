...
size_t IndexToOffset(const std::array<size_t, uDimNum> &dims, size_t &gap,
                     TCurIndexType curIdx, TRemainIndexType... remIdx) {
...
template <size_t ID, typename TShape> void FillShape(TShape &pShape) { return; }

template <size_t ID, typename TShape, typename TCurParam,
...
template <size_t uDimNum> class Shape {
...
public:
  ...
  template <typename... TIntTypes,
            std::enable_if_t<(std::is_convertible_v<TIntTypes, size_t> && ...)>
                * = nullptr>
  ...
  template <size_t vDimNum> bool operator==(const Shape<vDimNum> &) const {
    return false;
  }
  ...
  decltype(auto) begin() { return m_dims.begin(); }

  decltype(auto) begin() const { return m_dims.begin(); }

  decltype(auto) end() { return m_dims.end(); }

  decltype(auto) end() const { return m_dims.end(); }

  decltype(auto) rbegin() { return m_dims.rbegin(); }

  decltype(auto) rbegin() const { return m_dims.rbegin(); }

  decltype(auto) rend() { return m_dims.rend(); }

  decltype(auto) rend() const { return m_dims.rend(); }

  decltype(auto) size() const { return m_dims.size(); }
...
public:
  ...
  bool operator==(const Shape &val) const { return true; }
  ...
  decltype(auto) size() const { return 0; }
};
