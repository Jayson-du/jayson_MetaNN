  ...
  auto EvalRegister() const {
    if (!m_evalBuf.IsEvaluated()) {
      ...
      if (!EvalPlan::Inst().IsAlreadyRegisted(outHandle.DataPtr())) {
        ...
        using ItemType = NSTrivialTensor::EvalItem<decltype(handle),
                                                   decltype(outHandle), uDim>;
        using GroupType = NSTrivialTensor::EvalGroup<decltype(handle),
                                                     decltype(outHandle), uDim>;
        using DispatcherType = TrivialEvalItemDispatcher<GroupType>;

        auto item = std::make_unique<ItemType>(std::move(handle),
                                               std::move(outHandle), m_shape);
