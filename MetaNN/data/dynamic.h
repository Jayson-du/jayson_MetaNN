...
public:
  DynamicWrapper(TInternalData data) : m_internal(std::move(data)) {}
  ...
  DynamicConstEvalHandle<typename TBase::EvalType>
  EvalRegister() const override final {
    return m_internal.EvalRegister();
  }

  const TInternalData &Internal() const { return m_internal; }
...
public:
  ...
  template <typename T> const T *TryCastTo() const {
  ...
  bool IsEmpty() const { return m_internal == nullptr; }

private:
  std::shared_ptr<InternalType> m_internal;
};

template <typename TData> constexpr bool IsDynamic_ = false;
...
template <typename TData> auto MakeDynamic(TData &&data) {
  if constexpr (IsDynamic<TData>) {
    ...
    return DynamicData<typename RawData::ElementType,
                       typename RawData::DeviceType, DataCategory<RawData>>(
        std::move(internal));
  }
}
} // namespace MetaNN