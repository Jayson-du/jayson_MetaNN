  ...
  auto EvalRegister() const {
    if (!m_evalBuf.IsEvaluated()) {
      ...
      if (!EvalPlan::Inst().IsAlreadyRegisted(outHandle.DataPtr())) {
        ...
        using ItemType =
            NSBiasVector::EvalItem<decltype(handle), decltype(outHandle)>;
        using GroupType =
            NSBiasVector::EvalGroup<decltype(handle), decltype(outHandle)>;
        using DispatcherType = TrivialEvalItemDispatcher<GroupType>;

        auto item = std::make_unique<ItemType>(
            std::move(handle), std::move(outHandle), m_shape[0], m_pos);
