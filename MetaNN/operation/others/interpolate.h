...
template <typename TInputHandle1, typename TInputHandle2,
          typename TInputHandle3, typename TOutputHandle>
...
template <typename TInputHandle1, typename TInputHandle2,
          typename TInputHandle3, typename TOutputHandle>
class EvalGroup
    : public TrivialEvalGroup<EvalItem<TInputHandle1, TInputHandle2,
                                       TInputHandle3, TOutputHandle>> {
  using EvalItemType =
 ...
 protected:
  virtual void EvalInternalLogic(EvalItemType& evalItem) final override {
    ...
    for (size_t i = 0; i < outCount; ++i) {
      mem_out[i] = mem_in1[i % count1] * mem_in3[i % count3] +
                   mem_in2[i % count2] * (1 - mem_in3[i % count3]);
    }
