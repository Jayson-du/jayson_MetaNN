...
template <typename TInputHandle, typename TOutputHandle, typename TPolicy>
class EvalGroup
    : public TrivialEvalGroup<EvalItem<TInputHandle, TOutputHandle, TPolicy>> {
 ...
 protected:
  virtual void EvalInternalLogic(EvalItemType& evalItem) final override {
    ...
    size_t loopCount = 1;
    for (size_t i = 0;
         i < CategoryTagFromHandle<TOutputHandle>::DimNum - modDimNum; ++i) {
...
template <typename TGradHandle, typename TInputHandle, typename TOutputHandle,
          typename TPolicy>
class EvalGroup
    : public TrivialEvalGroup<
          EvalItem<TGradHandle, TInputHandle, TOutputHandle, TPolicy>> {
  using EvalItemType =
      EvalItem<TGradHandle, TInputHandle, TOutputHandle, TPolicy>;
  using ResType = typename TOutputHandle::DataType;
  using ElementType = typename ResType::ElementType;
...
class EvalItem : public BaseEvalItem {
 public:
  EvalItem(TGradHandle gradHandle, TWeightHandle weightHandle,
      ...
      : BaseEvalItem(TypeID<EvalItem>(),
                     {gradHandle.DataPtr(), weightHandle.DataPtr(),
                      softmaxHandle.DataPtr()},
                     outputHandle.DataPtr()),
        m_gradHandle(std::move(gradHandle)),
...
template <typename TGradHandle, typename TWeightHandle, typename TSoftmaxHandle,
          typename TOutputHandle, typename TPolicy>
class EvalGroup
    : public TrivialEvalGroup<EvalItem<
          TGradHandle, TWeightHandle, TSoftmaxHandle, TOutputHandle, TPolicy>> {
...
struct Calculator {
  ...
  static void EvalRegister(TEvalRes& evalRes, const TOp& oper) {
    if constexpr (!Valid_<typename TOp::template OperandType<0>,
      ...
      using TItem =
          EvalItem<RemConstRef<decltype(gradHandle)>,
                   RemConstRef<decltype(weightHandle)>,
                   RemConstRef<decltype(softmaxHandle)>,
                   RemConstRef<decltype(outHandle)>, typename TOp::Policies>;
      using TGroup =
          EvalGroup<RemConstRef<decltype(gradHandle)>,
                    RemConstRef<decltype(weightHandle)>,
                    RemConstRef<decltype(softmaxHandle)>,
                    RemConstRef<decltype(outHandle)>, typename TOp::Policies>;
      using EvalDispatcher = TrivialEvalItemDispatcher<TGroup>;

      auto item = std::make_unique<TItem>(
          std::move(gradHandle), std::move(weightHandle),
...
auto SoftmaxGrad(TGrad&& p_grad, TInput&& p_input) {
  ...
  using ResType =
      Operation<OpTags::SoftmaxGrad, OperandContainer<rawOp1, rawOp2>,
                PolicyContainer<PModifyDimNumIs<modDimNum>>>;
