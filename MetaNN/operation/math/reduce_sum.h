...
template <typename TPolicy, size_t uDimNum> struct DimArrToBit_ {
  ...
  using type =
      typename DimArrToBitHelper_<PDim,
...
template <typename TPolicy, size_t uDimNum> struct ModDimToBit_ {
  ...
  using type =
      typename ModDimToBitHelper_<ModDimNum,
                                  std::make_index_sequence<uDimNum>>::type;
};
...
class EvalGroup : public TrivialEvalGroup<
                      EvalItem<TInputHandle, TOutputHandle, TPolicies>> {
  ...
  virtual void EvalInternalLogic(EvalItemType &evalItem) final override {
    ...
    if constexpr (AimDim == 0) {
      ...
      for (size_t i = new_count; i < count; i += new_count) {
        std::transform(mem_in + i, mem_in + i + new_count, mem_out, mem_out,
...
struct OperCategory_<OpTags::ReduceSum, TPolicy, TOperand> {
  ...
  constexpr static size_t AccuBits = PolicySelect<DimPolicy, TPolicy>::IsKeepDim
                                         ? 0
                                         : OperReduceSum::AccuDimBits(DimBits);
  static_assert(OriDim >= AccuBits);
  using type = CategoryTags::Tensor<OriDim - AccuBits>;
};

template <typename TP>
constexpr bool IsValidOper<OpTags::ReduceSum, TP> =
...
auto ReduceSum(TP &&oper) {
  ...
  constexpr bool HasModDimNum =
      HasNonTrivialPolicy<TPolicy, DimPolicy, DimPolicy::ModifyDimNumValueCate>;

  static_assert((int)HasDimArray + (int)HasModDimNum <= 1,
                "only one of DimArray or ResDimValue could be set");

  using TDimBits = typename CompileTimeSwitch<
  ...
  if constexpr (OperReduceSum::IsTrivialDimBits(DimBits)) {
    ...
    using ResType =
        Operation<OpTags::ReduceSum, OperandContainer<rawOp>,
