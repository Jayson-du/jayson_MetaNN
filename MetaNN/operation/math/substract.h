...
template <typename TNumber, typename TOper> constexpr bool Valid() {
  if constexpr ((!IsValidCategoryTag<DataCategory<TOper>>) ||
    ...
    return std::is_constructible_v<typename RemConstRef<TOper>::ElementType,
                                   TNumber>;
  }
}
...
struct OperAuxParams<OpTags::SubstractFromNum, TElem, TCate>
...
template <typename TElem, typename TCate>
struct OperAuxParams<OpTags::SubstractByNum, TElem, TCate>
    : public OperAuxValue<TElem> {
  using TBase = OperAuxValue<TElem>;
  using TBase::TBase;
  using TBase::operator=;
...
auto operator-(TP1 &&p_m1, TP2 &&p_m2) {
  if constexpr (IsValidOper<OpTags::Substract, TP1, TP2>) {
    ...
    using ResType =
        Operation<OpTags::Substract, OperandContainer<rawOp1, rawOp2>>;
    ...
    using ResType =
        Operation<OpTags::SubstractFromNum, OperandContainer<rawOp>>;
    OperAuxParams<
        OpTags::SubstractFromNum, typename rawOp::ElementType,
        OperCateCal<OpTags::SubstractFromNum, PolicyContainer<>, rawOp>>
        params(p_m1);
    ...
    OperAuxParams<OpTags::SubstractByNum, typename rawOp::ElementType,
                  OperCateCal<OpTags::SubstractByNum, PolicyContainer<>, rawOp>>
        params(p_m2);
