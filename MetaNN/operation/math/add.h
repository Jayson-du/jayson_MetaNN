...
template <> struct OperSeq_<OpTags::Add> {
...
template <typename TOp1, typename TOp2> constexpr bool Valid() {
  if constexpr (IsValidCategoryTag<DataCategory<TOp1>> &&
    ...
    return std::is_constructible_v<typename RemConstRef<TOp1>::ElementType,
                                   TOp2>;
  } else if constexpr (!IsValidCategoryTag<DataCategory<TOp1>> &&
                       IsValidCategoryTag<DataCategory<TOp2>>) {
    return std::is_constructible_v<typename RemConstRef<TOp2>::ElementType,
                                   TOp1>;
  } else {
    return false;
  }
...
constexpr bool IsValidOper<OpTags::AddWithNum, TOp1, TOp2> =
    OperAddWithNum::Valid<TOp1, TOp2>();

template <typename TElem, typename TCate>
struct OperAuxParams<OpTags::AddWithNum, TElem, TCate>
    : public OperAuxValue<TElem> {
  using TBase = OperAuxValue<TElem>;
  using TBase::TBase;
  using TBase::operator==;
};

template <> struct OperSeq_<OpTags::AddWithNum> {
  using type =
      OperCalAlgoChain<TailCalculator<OperAddWithNum::NSCaseGen::EvalItem,
...
auto operator+(TP1 &&p_m1, TP2 &&p_m2) {
  if constexpr (IsValidOper<OpTags::Add, TP1, TP2>) {
    ...
    if constexpr (!IsValidCategoryTag<DataCategory<TP1>> &&
                  IsValidCategoryTag<DataCategory<TP2>>) {
      ...
      OperAuxParams<OpTags::AddWithNum, typename rawOp::ElementType,
                    OperCateCal<OpTags::AddWithNum, PolicyContainer<>, rawOp>>
          params(p_m1);
      return ResType(std::move(params), std::forward<TP2>(p_m2));
    } else if constexpr (IsValidCategoryTag<DataCategory<TP1>> &&
                         !IsValidCategoryTag<DataCategory<TP2>>) {
