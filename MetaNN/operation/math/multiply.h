...
constexpr bool IsValidOper<OpTags::MultiplyWithNum, TOp1, TOp2> =
    OperMultiplyWithNum::Valid<TOp1, TOp2>();

template <typename TElem, typename TCate>
struct OperAuxParams<OpTags::MultiplyWithNum, TElem, TCate>
...
auto operator*(TP1 &&p_m1, TP2 &&p_m2) {
  if constexpr (IsValidOper<OpTags::Multiply, TP1, TP2>) {
    ...
    using ResType =
        Operation<OpTags::Multiply, OperandContainer<rawOp1, rawOp2>>;
    ...
    if constexpr (!IsValidCategoryTag<DataCategory<TP1>> &&
      ...
      using ResType =
          Operation<OpTags::MultiplyWithNum, OperandContainer<rawOp>>;
      OperAuxParams<
          OpTags::MultiplyWithNum, typename rawOp::ElementType,
          OperCateCal<OpTags::MultiplyWithNum, PolicyContainer<>, rawOp>>
          params(p_m1);
