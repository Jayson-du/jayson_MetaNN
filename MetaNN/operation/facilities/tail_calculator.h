...
template <bool IsPassingPolicy, template <typename...> class EvalItem,
          template <typename...> class EvalGroup, typename TPolicy,
...
private:
  ...
  static auto DoEvalRegister(TOperHandleTuple operHandles, TResHandle resHandle,
    ...
    using ItemType = typename PickEvalTypes_<
        IsPassPolicy, EvalItem, EvalGroup, TPolicies,
        RemConstRef<decltype(std::get<Index>(operHandles))>...,
        RemConstRef<TResHandle>>::ItemType;
    using GroupType = typename PickEvalTypes_<
        IsPassPolicy, EvalItem, EvalGroup, TPolicies,
        RemConstRef<decltype(std::get<Index>(operHandles))>...,
        RemConstRef<TResHandle>>::GroupType;
    auto item = CreateEvalItem<
        PolicySelect<TailCalculatorPolicy, TPolicy>::IsPassShape,
        PolicySelect<TailCalculatorPolicy, TPolicy>::IsPassAuxParam, ItemType>(
        shape, auxParams, std::move(std::get<Index>(operHandles))...,
        std::move(resHandle));

    using TDispatcher =
        typename PolicySelect<TailCalculatorPolicy,
                              TPolicy>::template Dispatcher<GroupType>;
