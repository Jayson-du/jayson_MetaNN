...
constexpr bool IsValidOper<OpTags::Permute, TP> =
    (IsValidCategoryTag<DataCategory<TP>>)&&(DataCategory<TP>::DimNum > 1);
...
auto Permute(TP&& oper) {
  ...
  if constexpr (OperPermute::TrivialDims(dims)) {
    ...
    using PDim =
        PickPolicyOjbect<TDimPolicy, DimPolicy, DimPolicy::DimArrayValueCate>;
    using ResType = Operation<OpTags::Permute, OperandContainer<rawOp>,
                              PolicyContainer<PDim>>;
...
auto PermuteInv(TP&& oper) {
  ...
  if constexpr (OperPermute::TrivialDims(dims)) {
    ...
    using PDim =
        PickPolicyOjbect<TDimPolicy, DimPolicy, DimPolicy::DimArrayValueCate>;
    using PModDim =
        typename OperPermute::DimInv_<std::make_index_sequence<dims.size()>,
                                      PDim>::type;
