    ...
    if constexpr (TCate::DimNum == 0) {
      if (ori_shape_count != 1) {
        throw std::runtime_error(
            "Reshape error: cannot reshape to a scalar with more than 1 "
            "items.");
      }
    } else {
      ...
      for (size_t i = 0; i < m_shape.size(); ++i) {
        if (m_shape[i] == InferredDimSize) {
          if (inferred_pos == (size_t)-1) {
            ...
            throw std::runtime_error(
                "Reshape error: too many inferrence positions.");
          }
        } else
...
auto Reshape(TP &&oper, MetaNN::Shape<uDim> newShape) {
  ...
  OperAuxParams<OpTags::Reshape, typename ResType::ElementType,
                CategoryTags::Tensor<uDim>>
