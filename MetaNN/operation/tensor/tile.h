...
auto GetMuteShape(const std::array<bool, uAimDim>& dimBitArray,
  ...
  if (oriPos != uOriDim) {
    throw std::runtime_error(
        "GetMuteShape error, there might be duplicate array num in the input "
        "policy.");
    ;
  }
  return resShape;
...
struct OperCategory_<OpTags::Tile, TPolicy, TOp> {
  constexpr static auto dimBitArray =
      PolicySelect<DimPolicy, TPolicy>::DimBitArray;
...
auto Tile(TP&& oper, Shape<AimDim> aimShape) {
  ...
  if constexpr (HasNonTrivialPolicy<TPolicy, DimPolicy,
                                    DimPolicy::DimArrayValueCate>) {
    constexpr auto dimArray = PolicySelect<DimPolicy, TPolicy>::DimArray;
    static_assert(DataCategory<TP>::DimNum + dimArray.size() == AimDim,
                  "Shape size mismatch");
  }

  using TDimBits = typename std::conditional_t<
      HasDimArray, OperTile::DimArrToBit_<AimDim, TPolicy>,
      OperTile::DefaultToBit_<AimDim, DataCategory<TP>::DimNum>>::type;
  assert(OperTile::IsShapeCompatible(
      aimShape, OperTile::GetMuteShape(TDimBits::DimBitArray, oper.Shape())));

  using rawOp = RemConstRef<TP>;
  using ResType = Operation<OpTags::Tile, OperandContainer<rawOp>,
                            PolicyContainer<TDimBits>>;
  return ResType(
      OperAuxParams<OpTags::Tile, typename rawOp::ElementType,
