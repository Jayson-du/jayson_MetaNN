...
public:
    ...
    OperOrganizer(TInput&& p_input, TKernel&& p_kernel,
        ...
        : m_rowNum(CalculateOutSize(p_input.RowNum(),
                                    p_padHead.template
Get<ConvParams::RowNum>(), p_padTail.template Get<ConvParams::RowNum>(),
                                    p_strides.template
Get<ConvParams::RowNum>(), p_kernel.RowNum())) ,
m_colNum(CalculateOutSize(p_input.ColNum(), p_padHead.template
Get<ConvParams::ColNum>(), p_padTail.template Get<ConvParams::ColNum>(),
                                    p_strides.template
Get<ConvParams::ColNum>(), p_kernel.ColNum())) , m_pageNum(p_kernel.Length())
    {}
...
{
template <typename TIn, typename TKernel, typename TElem, typename TDevice,
typename TCategory> class EvalUnit;
...
struct Calculator
{
    ...
    static void EvalRegister(TEvalRes& evalRes, const TOper& oper)
    {
        ...
        UnitType unit(std::move(inputHandle), std::move(kernelHandle),
                      ...
                      oper.Ogranizer(),
                      std::move(outHandle));
        EvalPlan<DeviceType>::template Register<GroupType>(std::move(unit),
dataPtr, std::move(depVec));
...
namespace NSOperConv
{
    ...
    template <typename TInput, typename TKernel>
    constexpr bool valid = (IsThreeDArray<TInput> &&
IsThreeDArraySequence<TKernel>);
...
/// Convolution with "Default" padding mode
    ...
    static auto DefaultEval(TInput&& p_input, TKernel&& p_kernel,
                            TPadHeadValueCont&& p_padHead, TPadTailValueCont&&
p_padTail, TStrideValueCont&& p_strides)
    {
        ...
        static_assert(std::is_same_v<ElementType, typename
rawKernel::ElementType>, "Different element types cannot conv directly");
        static_assert(std::is_same_v<DeviceType, typename
rawKernel::DeviceType>, "Different device types cannot conv directly");

        if (p_input.PageNum() != p_kernel.PageNum())
        {
            throw std::runtime_error("The input and kernel should have same
depth!");
        }
        ...
        using ResType = BinaryOp<ConvRelated::Conv2D,
                                 RemConstRef<TInput>,
                                 RemConstRef<TKernel>>;
        return ResType(std::forward<TInput>(p_input),
std::forward<TKernel>(p_kernel), std::forward<TPadHeadValueCont>(p_padHead),
...
/// Convolution with "Same" padding mode
    ...
    static auto SameEval(TInput&& p_input, TKernel&& p_kernel,
                         TStrideValueCont&& p_strides)
    {
        const size_t rowPad = CalculatePadSize(p_input.RowNum(),
p_strides.template Get<ConvParams::RowNum>(), p_kernel.RowNum()); const size_t
colPad = CalculatePadSize(p_input.ColNum(), p_strides.template
Get<ConvParams::ColNum>(), p_kernel.ColNum());
        ...
        auto padHead = VarTypeDict<ConvParams::RowNum,
ConvParams::ColNum>::Create() .template Set<ConvParams::RowNum>(rowPadHead)
                        .template Set<ConvParams::ColNum>(colPadHead);
        auto padTail = VarTypeDict<ConvParams::RowNum,
ConvParams::ColNum>::Create() .template Set<ConvParams::RowNum>(rowPadTail)
                        .template Set<ConvParams::ColNum>(colPadTail);
        return DefaultEval(std::forward<TInput>(p_input),
std::forward<TKernel>(p_kernel), std::move(padHead), std::move(padTail),
                           std::forward<TStrideValueCont>(p_strides));
    }
};
...
auto DefaultConv(TInput&& p_input, TKernel&& p_kernel,
                 ...
                 TStrideValueCont&& p_strides)
{
    return NSOperConv::DefaultEval(std::forward<TInput>(p_input),
std::forward<TKernel>(p_kernel), std::forward<TPadHeadValueCont>(p_padHead),
                                 std::forward<TPadTailValueCont>(p_padTail),
                                 std::forward<TStrideValueCont>(p_strides));
}
...
auto SameConv(TInput&& p_input, TKernel&& p_kernel,
              TStrideValueCont&& p_strides)
{
    return NSOperConv::SameEval(std::forward<TInput>(p_input),
std::forward<TKernel>(p_kernel), std::forward<TStrideValueCont>(p_strides));
}
}*/