...
public:
  ...
  DynamicConstEvalHandle(TRealHandle data)
      : m_data(std::make_shared<NSEvalHandle::DynamicHandleData<TRealHandle>>(
            std::move(data))) {
    assert(m_data);
  }
...
using DeviceTypeFromHandle =
    typename RemConstRef<decltype(std::declval<THandle>().Data())>::DeviceType;

template <typename THandle>
using ElementTypeFromHandle =
    typename RemConstRef<decltype(std::declval<THandle>().Data())>::ElementType;
