...
using CommonInputMap =
    LayerInMap<LayerKV<LeftOperand, Matrix<CheckElement, CheckDevice>>,
...
void test_substract_layer1() {
  ...
  auto input =
      LayerInputCont<RootLayer>().Set<LeftOperand>(i1).Set<RightOperand>(i2);
...
void test_substract_layer2() {
  ...
  using RootLayer =
      MakeTrainLayer<SubstractLayer, CommonInputMap, PFeedbackOutput>;
  static_assert(RootLayer::IsFeedbackOutput, "Test Error");
  static_assert(!RootLayer::IsUpdate, "Test Error");
...
void test_substract_layer3() {
  ...
  input =
      LayerInputCont<RootLayer>().Set<LeftOperand>(i3).Set<RightOperand>(i4);
...
void test_substract_layer4() {
  ...
  auto input =
      LayerInputCont<RootLayer>().Set<LeftOperand>(i1).Set<RightOperand>(3.3f);

  auto out = layer.FeedForward(input);
  auto res = Evaluate(out.Get<LayerOutput>());
  for (size_t i = 0; i < 2; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      // Note: since RightOperand should be int, the 3.3f should be translated
      // into 3.
  ...
  auto handle1 = out_grad.Get<LeftOperand>().EvalRegister();
  static_assert(
      std::is_same_v<RemConstRef<decltype(out_grad.Get<RightOperand>())>,
                     NullParameter>);
  EvalPlan::Inst().Eval();
...
void test_substract_layer5() {
  ...
  using InputMap =
      LayerInMap<LayerKV<LeftOperand, int>,
                 LayerKV<RightOperand, Matrix<CheckElement, CheckDevice>>>;
  ...
  auto input =
      LayerInputCont<RootLayer>().Set<LeftOperand>(3.3f).Set<RightOperand>(i1);
  ...
  static_assert(
      std::is_same_v<RemConstRef<decltype(out_grad.Get<LeftOperand>())>,
                     NullParameter>);
  EvalPlan::Inst().Eval();
