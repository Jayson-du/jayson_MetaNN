...
void test_sigmoid_layer2() {
  ...
  using RootLayer =
      MakeTrainLayer<SigmoidLayer, CommonInputMap, PFeedbackOutput>;
  static_assert(RootLayer::IsFeedbackOutput, "Test Error");
  static_assert(!RootLayer::IsUpdate, "Test Error");
  ...
  assert(fabs(fb(0, 0) -
              0.1f * exp(0.27f) / (1 + exp(0.27f)) / (1 + exp(0.27f))) < 0.001);
  assert(fabs(fb(1, 0) -
              0.3f * exp(0.41f) / (1 + exp(0.41f)) / (1 + exp(0.41f))) < 0.001);
...
void test_sigmoid_layer3() {
  ...
  for (size_t loop_count = 9; loop_count >= 1; --loop_count) {
    ...
    for (size_t i = 0; i < loop_count; ++i) {
      for (size_t j = 0; j < 3; ++j) {
        CheckElement aim =
            exp(-in(i, j)) / (1 + exp(-in(i, j))) / (1 + exp(-in(i, j)));
