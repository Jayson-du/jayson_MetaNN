...
namespace {
using CommonInputMap = LayerInMap<
    LayerKV<InterpolateLayerWeight1, Matrix<CheckElement, CheckDevice>>,
...
void test_interpolate_layer1() {
  ...
  auto input = LayerInputCont<RootLayer>()
                   .Set<InterpolateLayerWeight1>(i1)
  ...
  static_assert(
      decltype(out_grad)::template IsValueEmpty<InterpolateLayerWeight1>);
  static_assert(
      decltype(out_grad)::template IsValueEmpty<InterpolateLayerWeight2>);
  static_assert(
      decltype(out_grad)::template IsValueEmpty<InterpolateLayerLambda>);
...
void test_interpolate_layer2() {
  ...
  using RootLayer =
      MakeTrainLayer<InterpolateLayer, CommonInputMap, PFeedbackOutput>;
  ...
  for (size_t loop_count = 9; loop_count >= 1; --loop_count) {
    ...
    for (size_t i = 0; i < loop_count; ++i) {
      for (size_t j = 0; j < 3; ++j) {
        ...
        assert(fabs(rlambda(i, j) - grad(i, j) * (i1(i, j) - i2(i, j))) <
               0.00001f);
      }
    }
  }
