...
using MultiDimInputMap =
    LayerInMap<LayerKV<LeftOperand, Tensor<CheckElement, CheckDevice, 3>>,
...
void test_dot_layer6() {
  ...
  using RootLayer = MakeTrainLayer<DotLayer, MultiDimInputMap, PFeedbackOutput,
                                   PModifyDimNumIs<2>>;
  ...
  auto grad1 = Evaluate(Dot<PolicyContainer<PModifyDimNumIs<2>>>(
      grad, Permute<PolicyContainer<PDimArrayIs<2, 3, 0, 1>>>(i2)));
  ...
  auto grad2 = Evaluate(Dot<PolicyContainer<PModifyDimNumIs<1>>>(
      Permute<PolicyContainer<PDimArrayIs<1, 2, 0>>>(i1), grad));
