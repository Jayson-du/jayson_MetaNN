...
void test_nll_loss_layer1() {
  ...
  auto input =
      LayerInputCont<RootLayer>().Set<LayerInput>(in).Set<LossLayerWeight>(
          weight);
...
void test_nll_loss_layer2() {
  ...
  using RootLayer =
      MakeTrainLayer<NLLLossLayer, CommonInputMap, PFeedbackOutput>;
  static_assert(RootLayer::IsFeedbackOutput, "Test Error");
  static_assert(!RootLayer::IsUpdate, "Test Error");
  ...
  auto fb = LayerOutputCont<RootLayer>().Set<LayerOutput>(
      Scalar<CheckElement, CheckDevice>(0.5));
...
void test_nll_loss_layer3() {
  ...
  for (size_t loop_count = 9; loop_count >= 1; --loop_count) {
    auto out_grad =
        layer.FeedBackward(LayerOutputCont<RootLayer>().Set<LayerOutput>(
            Scalar<CheckElement, CheckDevice>(0.5 * loop_count)));
    auto fb = Evaluate(out_grad.Get<LayerInput>());

    auto in = op_in.back();
    op_in.pop_back();
    auto weight = op_weight.back();
    op_weight.pop_back();
    ...
    for (size_t i = 0; i < loop_count * 2; ++i) {
      for (size_t j = 0; j < 4; ++j) {
        assert(fabs(fb(i, j) +
                    0.5 * loop_count * weight(i, j) / in(i, j) / sum) < 0.0001);
      }
    }
  }
