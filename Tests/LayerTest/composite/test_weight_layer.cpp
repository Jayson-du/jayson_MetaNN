...
using CommonInputMap =
    LayerInMap<LayerKV<LayerInput, Matrix<CheckElement, CheckDevice>>>;

void test_weight_layer1() {
  cout << "Test weight layer case 1 ...\t" << flush;
  using RootLayer =
      MakeInferLayer<WeightLayer,
                     PParamTypeIs<Matrix<CheckElement, CheckDevice>>>;
...
void test_weight_layer2() {
  ...
  using RootLayer =
      MakeTrainLayer<WeightLayer, CommonInputMap, PUpdate,
                     PParamTypeIs<Matrix<CheckElement, CheckDevice>>>;
  static_assert(!RootLayer::IsFeedbackOutput);
  static_assert(RootLayer::IsUpdate);
  ...
  auto out_grad =
      layer.FeedBackward(LayerOutputCont<RootLayer>().Set<LayerOutput>(g));
  static_assert(decltype(out_grad)::template IsValueEmpty<LayerInput>);
...
void test_weight_layer3() {
  ...
  using RootLayer =
      MakeTrainLayer<WeightLayer, CommonInputMap, PUpdate, PFeedbackOutput,
                     PParamTypeIs<Matrix<CheckElement, CheckDevice>>>;
  static_assert(RootLayer::IsFeedbackOutput);
  static_assert(RootLayer::IsUpdate);
...
void test_weight_layer4() {
  ...
  for (int loop_count = 0; loop_count < 10; ++loop_count) {
    ...
    auto out =
        layer.FeedForward(LayerInputCont<RootLayer>().Set<LayerInput>(input));
  ...
  for (int loop_count = 1; loop_count < 10; ++loop_count) {
    aim =
        Evaluate(aim + Dot(Transpose(op_in[loop_count]), op_grad[loop_count]));
  }
...
void test_weight_layer5() {
  ...
  using RootLayer =
      MakeTrainLayer<WeightLayer, CommonInputMap, PUpdate, PFeedbackOutput,
                     PInitializerIs<RootFiller>,
                     PParamTypeIs<Matrix<CheckElement, CheckDevice>>>;
  RootLayer layer("root", 800, 400);

  auto initializer = MakeInitializer<CheckElement>(
      InitializerKV<RootFiller>(UniformFiller{-1, 1}));
  LoadBuffer<CheckElement, CheckDevice> loadBuffer;
  ...
  cout << "mean-delta = " << fabs(mean)
       << " Variance-delta = " << fabs(var - 0.333) << ' ';
  cout << "done" << endl;
}
...
void test_weight_layer6() {
  ...
  auto initializer = MakeInitializer<CheckElement>(
      InitializerKV<RootFiller>(UniformFiller{-1.5, 1.5}));
  LoadBuffer<CheckElement, CheckDevice> loadBuffer;
  LayerInit(layer, initializer, loadBuffer);
  assert(loadBuffer.IsParamExist<CategoryTags::Matrix>("root/param"));
