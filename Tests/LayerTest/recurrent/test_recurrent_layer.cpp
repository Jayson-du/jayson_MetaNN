...
using Topology =
    ComposeTopology<Sublayer<AddSublayer, AddLayer>,
...
using Topology = ComposeTopology<
    Sublayer<AddSublayer1, AddLayer>, Sublayer<AddSublayer2, AddLayer>,
...
void test_recurrent_layer1() {
  ...
  using RootLayer =
      MakeInferLayer<RecurrentLayer, PSeqIDsAre<SeqID<LayerInput, 0>>,
                     PActFuncIs<AddWrapLayer>>;
...
using SeqInputMap = LayerInMap<
    LayerKV<LayerInput, Tensor<CheckElement, CheckDevice, 3>>,
...
void test_recurrent_layer2() {
  ...
  using RootLayer = MakeTrainLayer<RecurrentLayer, SeqInputMap,
...
void test_recurrent_layer3() {
  ...
  using RootLayer =
      MakeTrainLayer<RecurrentLayer, SeqInputMap, PActFuncIs<AddWrapLayer>,
                     PSeqIDsAre<SeqID<LayerInput, 0>>, PFeedbackOutput>;
  static_assert(!RootLayer::IsUpdate);
  static_assert(RootLayer::IsFeedbackOutput);
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      ...
      assert(fabs(grad_input[1](j, k) - grad[1](j, k) - grad[2](j, k)) <
             0.001f);
      assert(fabs(grad_input(0, j, k) - grad(0, j, k) - grad(1, j, k) -
                  grad(2, j, k)) < 0.001f);
...
void test_recurrent_layer4() {
  ...
  using RootLayer =
      MakeInferLayer<RecurrentLayer, PSeqIDsAre<SeqID<LeftOperand, 0>>,
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(0, j, k) - lInput(0, j, k) - rInput(j, k) - prev(j, k)) <
             0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(1, j, k) - lInput(1, j, k) - rInput(j, k) -
                  res(0, j, k)) < 0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(2, j, k) - lInput(2, j, k) - rInput(j, k) -
                  res(1, j, k)) < 0.001f);
    }
  }
...
void test_recurrent_layer5() {
  ...
  using RootLayer =
      MakeInferLayer<RecurrentLayer, PSeqIDsAre<SeqID<LeftOperand, 1>>,
                     PActFuncIs<AddWrapLayer2>>;
  static_assert(!RootLayer::IsUpdate);
  static_assert(!RootLayer::IsFeedbackOutput);
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(0, j, k) - lInput(j, 0, k) - rInput(j, k) - prev(j, k)) <
             0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(1, j, k) - lInput(j, 1, k) - rInput(j, k) -
                  res(0, j, k)) < 0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(2, j, k) - lInput(j, 2, k) - rInput(j, k) -
                  res(1, j, k)) < 0.001f);
    }
  }
...
void test_recurrent_layer6() {
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(0, j, k) - lInput(j, 0, k) - rInput(j, k, 0) -
                  prev(j, k)) < 0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(1, j, k) - lInput(j, 1, k) - rInput(j, k, 1) -
                  res(0, j, k)) < 0.001f);
    }
  }
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      assert(fabs(res(2, j, k) - lInput(j, 2, k) - rInput(j, k, 2) -
                  res(1, j, k)) < 0.001f);
    }
  }
...
using SeqInputMap2 = LayerInMap<
    LayerKV<LeftOperand, Tensor<CheckElement, CheckDevice, 3>>,
...
void test_recurrent_layer7() {
  ...
  using RootLayer = MakeTrainLayer<RecurrentLayer, SeqInputMap2,
                                   PSeqIDsAre<SeqID<LeftOperand, 0>>,
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      ...
      assert(fabs(gradL(0, j, k) - grad(0, j, k) - grad(1, j, k) -
                  grad(2, j, k)) < 0.001f);
      assert(fabs(gradR(j, k) - grad(0, j, k) - 2 * grad(1, j, k) -
                  3 * grad(2, j, k)) < 0.001f);
...
void test_recurrent_layer8() {
  cout << "Test recurrent layer case 8 (2 inputs, non-zero seq ID, out "
          "grad)...\t";
  using RootLayer = MakeTrainLayer<RecurrentLayer, SeqInputMap2,
                                   PSeqIDsAre<SeqID<LeftOperand, 1>>,
                                   PActFuncIs<AddWrapLayer2>, PFeedbackOutput>;
  static_assert(!RootLayer::IsUpdate);
  static_assert(RootLayer::IsFeedbackOutput);
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      ...
      assert(fabs(gradL(j, 0, k) - grad(0, j, k) - grad(1, j, k) -
                  grad(2, j, k)) < 0.001f);
      assert(fabs(gradR(j, k) - grad(0, j, k) - 2 * grad(1, j, k) -
                  3 * grad(2, j, k)) < 0.001f);
...
using SeqInputMap3 = LayerInMap<
    LayerKV<LeftOperand, Tensor<CheckElement, CheckDevice, 3>>,
    LayerKV<RightOperand, Tensor<CheckElement, CheckDevice, 3>>,
    LayerKV<Previous<LayerOutput>, Matrix<CheckElement, CheckDevice>>>;
void test_recurrent_layer9() {
  cout << "Test recurrent layer case 9 (2 inputs, non-zero seq ID, output "
          "grad)...\t";
  using RootLayer =
      MakeTrainLayer<RecurrentLayer, SeqInputMap3,
                     PSeqIDsAre<SeqID<LeftOperand, 1>, SeqID<RightOperand, 2>>,
                     PActFuncIs<AddWrapLayer2>, PFeedbackOutput>;
  static_assert(!RootLayer::IsUpdate);
  static_assert(RootLayer::IsFeedbackOutput);
  ...
  for (size_t j = 0; j < 5; ++j) {
    for (size_t k = 0; k < 7; ++k) {
      ...
      assert(fabs(gradR(j, k, 0) - grad(0, j, k) - grad(1, j, k) -
                  grad(2, j, k)) < 0.001f);
