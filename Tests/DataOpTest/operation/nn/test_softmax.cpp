...
void test_softmax_case3() {
  ...
  for (size_t b = 0; b < 14; ++b) {
    ...
    for (size_t i = 0; i < 10; ++i) {
      assert(fabs(t_r(b / 2, b % 2, i) - exp(rm1(b / 2, b % 2, i)) / sum) <
             0.0001);
    }
  }
  cout << "done" << endl;
...
void test_softmax_case4() {
  ...
  for (size_t b = 0; b < 7; ++b) {
    ...
    for (size_t i = 0; i < 20; ++i) {
      assert(fabs(t_r(b, i / 10, i % 10) - exp(rm1(b, i / 10, i % 10)) / sum) <
             0.0001);
    }
  }
  cout << "done" << endl;
...
void test_softmax_grad_case4() {
  cout << "Test softmax grad case 4 (NLL with general weight vector, bacth "
          "mode) ...\t";
