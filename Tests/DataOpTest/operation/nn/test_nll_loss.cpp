...
void test_nll_loss_case3() {
  ...
  auto pred =
      GenTensor<CheckElement>(1, static_cast<CheckElement>(0.1), 3, 10, 7);
  auto op = NLLLoss(truth, pred);
  static_assert(IsScalar<decltype(op)>);
...
void test_nll_loss_grad1() {
  ...
  for (size_t i = 0; i < 4; ++i) {
    for (size_t j = 0; j < 5; ++j) {
      assert(fabs(div_r(i, j) + 0.5 * rm1(i, j) / rm2(i, j) / checkSum) <
             0.001);
    }
  }
...
void test_nll_loss_grad3() {
  ...
  for (size_t b = 0; b < 4; ++b) {
    for (size_t i = 0; i < 5; ++i) {
      for (size_t j = 0; j < 7; ++j) {
        assert(fabs(div_r(b, i, j) +
                    0.5 * rm1(b, i, j) / rm2(b, i, j) / checkSum) < 0.001);
      }
    }
  }
  cout << "done" << endl;
